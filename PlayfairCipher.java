...
public class PlayfairCipher {
    ...
    // Step 1: Clean the key (remove duplicates, replace J with I)
    ...
    // Step 2: Generate the 5x5 key matrix (FIXED)
    private void generateMatrix() {
        boolean[] seen = new boolean[26];
        seen['J' - 'A'] = true; // 'J' and 'I' share a cell
        
        StringBuilder fullKey = new StringBuilder();

        // Add key characters first and mark them as seen
        for (int i = 0; i < key.length(); i++) {
            char c = key.charAt(i);
            if (!seen[c - 'A']) {
                seen[c - 'A'] = true;
    ...
    // Step 3: Format plaintext (handle duplicate consecutive letters and odd lengths)
    private String cleanPlainText(String pt) {
        ...
        for (int i = 0; i < sb.length() - 1; i += 2) {
            if (sb.charAt(i) == sb.charAt(i + 1)) {
                sb.insert(i + 1, 'X'); // Insert 'X' between identical consecutive letters
            }
        }

        if (sb.length() % 2 != 0) {
            sb.append('Z'); // Append 'Z' if length is odd
        }
        return sb.toString();
    }

    // Find row and column coordinates of a character in the matrix
